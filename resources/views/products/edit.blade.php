...
<head>
    ...
    <title>Modifier {{ $product['nom'] }}</title>
    <style>
        body { font-family: Arial, sans-serif; margin: 30px; }
        form { max-width: 500px; }
        label { display: block; margin-top: 12px; font-weight: bold; }
        input, textarea { width: 100%; padding: 8px; margin-top: 6px; box-sizing: border-box; }
        .btn { display: inline-block; margin-top: 16px; padding: 10px 16px; background: #d08a1d; color: white; border: none; border-radius: 4px; cursor: pointer; text-decoration: none; }
...
<body>
    ...
    <form action="{{ route('products.update', $product['id']) }}" method="POST">
        ...
        <input type="text" id="nom" name="nom" value="{{ old('nom', $product['nom']) }}" required>

        <label for="description">Description</label>
        <textarea id="description" name="description" required>{{ old('description', $product['description']) }}</textarea>

        <label for="prix">Prix</label>
        <input type="number" id="prix" name="prix" step="0.01" value="{{ old('prix', $product['prix']) }}" required>
